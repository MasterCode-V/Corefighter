...
from app.models import Article, Persona, Purchase, PurchaseImage
...
async def create_purchase(
    ...
    purchase = Purchase(created_by=current_user.id, **body.model_dump())
    db.add(purchase)
    await db.commit()
    await db.refresh(purchase, attribute_names=["images"])
    return purchase
...
async def list_purchases(
    ...
    stmt = select(Purchase).options(selectinload(Purchase.images))
